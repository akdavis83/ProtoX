#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Quantum Coin Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/qtc-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Quantum Coin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Quantum Coin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example qtc.conf file. if contrib/devtools/gen-qtc-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/qtc.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/Quantum Coin-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
