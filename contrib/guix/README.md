# Bootstrappable Quantum Coin Core Builds

This directory contains the files necessary to perform bootstrappable Quantum Coin
Core builds.
...
To build Quantum Coin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Quantum Coin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/qtc-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/qtc-core/guix.sigs).
