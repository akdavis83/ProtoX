...
Tracing (USDT) support for Quantum Coin Core on Linux using. For more information on
USDT support in Quantum Coin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `qtcd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `qtcd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Quantum Coin Core USDT documentation] on how to list available tracepoints in your
`qtcd` binary.

[Quantum Coin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof qtcd)
```
...
Quantum Coin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof qtcd)
```
...
In a different terminal, starting Quantum Coin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/qtcd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof qtcd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof qtcd)
```
