...
Provides utilities for managing copyright headers of `The Quantum Coin Core
...
Generates a qtc.conf file in `share/examples/` by parsing the output from `qtcd --help`. This script is run during the
release process to include a qtc.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-qtc-conf.sh
```
