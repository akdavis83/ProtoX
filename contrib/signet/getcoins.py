...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing qtc-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='qtc-cli', help='qtc-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Quantum Coin address to which the faucet should send')
...
parser.add_argument('qtc_cli_args', nargs='*', help='Arguments to pass on to qtc-cli (default: -signet)')

args = parser.parse_args()

if args.qtc_cli_args == []:
    args.qtc_cli_args = ['-signet']


def qtc_cli(rpc_command_and_params):
    argv = [args.cmd] + args.qtc_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = qtc_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = qtc_cli(['getnewaddress', 'faucet', 'bech32'])
