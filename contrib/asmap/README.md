...
be used in Quantum Coin Core with the `-asmap` option.
...
to binary before being used with Quantum Coin Core.
...
the `qtc-cli getnodeaddresses` command.
...
```
qtc-cli getnodeaddresses 0 > addrs.json
```
