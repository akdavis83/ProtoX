...
"""Script for verifying Quantum Coin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from qtccore.org and qtc.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/qtc-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://qtccore.org"
HOST2 = "https://qtc.org"
VERSIONPREFIX = "qtc-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Quantum Coin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    ...
    # remove binaries that are known not to be hosted by qtccore.org
...
def main():
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes qtc.org lags behind qtccore.org.)')
    )
