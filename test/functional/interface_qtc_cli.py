...
# The block reward of coinbaseoutput.nValue (50) QTC/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 QTC/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"qtc-cli -h\" for help or \"qtc-cli listwallets\" to see "
...
class TestQuantum CoinCli(Quantum CoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `qtc-cli getblockchaininfo`")
        ...
        # Prevent qtc-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo and qtc-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and qtc-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (QTC/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (QTC/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 QTC) to the default wallet.
            ...
            # Test qtc-cli -generate with -rpcwallet in multiwallet mode.
...
if __name__ == '__main__':
    TestQuantum CoinCli(__file__).main()
