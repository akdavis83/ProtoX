...
class SimulateTxTest(Quantum CoinTestFramework):
    ...
    def run_test(self):
        ...
        qtc_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 btc decrease, w2 sees + 5 btc
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + qtc_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - qtc_fee + 4.9999       -5 - qtc_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - qtc_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - qtc_fee)
        ...
        tx2 = funding["hex"]
        qtc_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + qtc_fee2))
