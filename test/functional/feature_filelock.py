...
"""Check that it's not possible to start a second qtcd instance using the same datadir or wallet."""
import random
import string
...
class FilelockTest(Quantum CoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second qtcd instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second qtcd instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second qtcd using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second qtcd instance shutdown
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second qtcd instance using the same wallet")
