...
Quantum Coin Core version 0.20.0 is now available from:

  <https://qtccore.org/bin/qtc-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Quantum Coin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Quantum Coin Core version 0.18.0.
...
- Testing or debugging of implementations of the Quantum Coin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Quantum Coin Core. Quantum Coin Core logs debug messages
...
- OpenSSL is no longer used by Quantum Coin Core.  (#17265)

- BIP70 support has been fully removed from Quantum Coin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/qtc/qtc/issues/16599),
  [PR #16702](https://github.com/qtc/qtc/pull/16702), and the
  `qtcd help` for more information.  This option is experimental and
...
- All custom settings configured when Quantum Coin Core starts are now
...
- The "Start Quantum Coin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://qtccore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Quantum Coin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Quantum Coin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/qtc-core/HWI) for
  signing.  Future versions of Quantum Coin Core should support a GUI option
...
- Quantum Coin Core's automatically-generated source code documentation is
  now available at https://doxygen.qtccore.org.  (#17596)
...
- The `qtc-cli` utility used with the `-getinfo` parameter now
...
- The `qtc-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `qtcstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_qtc-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_qtc` binary (hebasto)
...
- #18702 Fix ASLR for qtc-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate qtc schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest qtc.conf (sdaftuar)
...
- #18022 Fix appveyor `test_qtc` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (qtc-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone qtc-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Quantum Coin Core files (hebasto)
...
- #17119 Fix broken qtc-cli examples (andrewtoth)
...
- #17503 Remove bitness from qtc-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.qtccore.org (laanwj)
...
- #17648 Rename wallet-tool references to qtc-wallet (hel-o)
...
- #18341 Replace remaining literal QTC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On qtcd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from qtc-cli -getinfo (malevolent)
