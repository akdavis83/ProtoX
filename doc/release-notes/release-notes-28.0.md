Quantum Coin Core version 28.0 is now available from:

  <https://qtccore.org/bin/qtc-core-28.0/>
...
Running Quantum Coin Core binaries on macOS requires self signing.
```
cd /path/to/qtc-28.0/bin
xattr -d com.apple.quarantine qtc-cli qtc-qt qtc-tx qtc-util qtc-wallet qtcd test_qtc
codesign -s - qtc-cli qtc-qt qtc-tx qtc-util qtc-wallet qtcd test_qtc
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Quantum Coin
...
Support for Testnet4 as specified in [BIP94](https://github.com/qtc/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Quantum Coin`
to `C:\Users\Username\AppData\Local\Quantum Coin`. Quantum Coin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/qtc/qtc/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libqtcconsensus Removal
---------------------------

The libqtc-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Quantum Coin Core was listening for P2P connections, either using
...
- Quantum Coin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/qtc/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Quantum Coin Core with the configuration
...
are now required to compile Quantum Coin Core. (#29091, #30263)

- The minimum required glibc to run Quantum Coin Core is now
...
- A new `-testdatadir` option has been added to `test_qtc` to allow specifying the
...
Previous releases of Quantum Coin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
