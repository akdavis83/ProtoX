# User-space, Statically Defined Tracing (USDT) for Quantum Coin Core

Quantum Coin Core includes statically defined tracepoints to allow for more
...
**Note**: Quantum Coin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Quantum Coin Core
...
Multiple tools can list the available tracepoints in a `qtcd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Quantum Coin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/qtcd
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/qtcd
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/qtcd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Quantum Coin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/qtcd | grep NT_STAPSDT -A 4 -B 2
