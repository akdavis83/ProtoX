...
The Quantum Coin Core daemon can be configured to act as a trusted "border
router", implementing the qtc wire protocol and relay, making
...
The ZeroMQ feature in Quantum Coin Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ qtcd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               ...
               -zmqpubrawtx=ipc:///tmp/qtcd.tx.raw \
...
From the perspective of qtcd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into qtcd directly. Furthermore, no information is
...
using. Quantum Coind appends an up-counting sequence number to each
