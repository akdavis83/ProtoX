# CJDNS support in Quantum Coin Core

It is possible to run Quantum Coin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Quantum Coin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Quantum Coin network and individual nodes.
...
before it will work with your Quantum Coin Core node. This is described in steps
...
## Run Quantum Coin Core with CJDNS

Once you are connected to the CJDNS network, the following Quantum Coin Core
...
When enabled, this option tells Quantum Coin Core that it is running in an
...
IPv6 local network. This helps Quantum Coin Core perform better address management:
...
CJDNS support was added to Quantum Coin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `qtc-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/qtc/qtc/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Quantum Coin Core

There are several ways to see your CJDNS address in Quantum Coin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `qtc-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `qtc-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `qtc-cli help getnodeaddresses` for details.
