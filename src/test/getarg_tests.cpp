...
void ResetArgs(ArgsManager& local_args, const std::string& strArg)
{
    ...
    vecArg.insert(vecArg.begin(), "testqtc");
...
BOOST_AUTO_TEST_CASE(patharg)
{
    ...
    const fs::path absolute_path{"/home/user/.qtc"};
    ResetArgs(local_args, "-dir=/home/user/.qtc");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);

    ResetArgs(local_args, "-dir=/root/../home/user/.qtc");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);
    ...
    const fs::path relative_path{"user/.qtc"};
    ResetArgs(local_args, "-dir=user/.qtc");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);

    ResetArgs(local_args, "-dir=somewhere/../user/.qtc");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);
    ...
    // line arguments or qtc.conf values. Currently the -dir= case cannot be
