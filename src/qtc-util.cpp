...
static void SetupQuantum CoinUtilArgs(ArgsManager &argsman)
{
...
{
    SetupQuantum CoinUtilArgs(args);
    std::string error;
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " qtc-util utility version " + FormatFullVersion() + "\n";

        if (args.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The qtc-util tool provides qtc related functionality that does not rely on the ability to access a running node. Available [commands] are listed below.\n"
                "\n"
                "Usage:  qtc-util [options] [command]\n"
                "or:     qtc-util [options] grind <hex-block-header>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
