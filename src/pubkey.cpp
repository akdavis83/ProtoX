 ...
 *  Quantum Coin because since the activation of BIP66, signatures are verified to be
...
// QTC Quantum-Safe Public Key Implementation
QCompressedPubKey::QCompressedPubKey(const std::vector<unsigned char>& full_pubkey) {
    m_hash = QHashPubKey(full_pubkey);
}
...
bool CPubKey::Verify(const uint256 &hash, const std::vector<unsigned char>& vchSig) const {
     ...
     * not historically been enforced in Quantum Coin, so normalize them first. */
