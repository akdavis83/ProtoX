This directory contains the source code for the Quantum Coin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Quantum Coin UI.
...
- ClientModel is used by the main application `qtcgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a qtc:... link from a browser).
...
* UI elements like Quantum CoinAmountField, which inherit from QWidget.
* `qtcstrings.cpp`: automatically generated
* `qtcunits.(h/cpp)`: QTC / mQTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/qtc_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `qtc-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `qtc-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `qtc-qt`
9. You're all set! Start developing, building, and debugging the Quantum Coin Core GUI
