                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<Quantum CoinAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = Quantum CoinUnits::formatWithUnit(unit, expected_balance, false, Quantum CoinUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_qtc-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_qtc-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_qtc-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    Quantum CoinAmountField* amountInput = receiveCoinsDialog.findChild<Quantum CoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_qtc-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
