...
/** Call node.updateRwSetting() with Quantum Coin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Quantum Coin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/qtc/qtc/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because qtc 22.x will not try to read these.
...
//! Convert enabled/size values to qtc -prune setting.
...
//! Get pruning enabled value to show in GUI from qtc -prune setting.
...
//! Get pruning size value to show in GUI from qtc -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayQuantum CoinUnit")) {
        settings.setValue("DisplayQuantum CoinUnit", QVariant::fromValue(Quantum CoinUnit::QTC));
    }
    QVariant unit = settings.value("DisplayQuantum CoinUnit");
    if (unit.canConvert<Quantum CoinUnit>()) {
        m_display_qtc_unit = unit.value<Quantum CoinUnit>();
    } else {
        m_display_qtc_unit = Quantum CoinUnit::QTC;
        settings.setValue("DisplayQuantum CoinUnit", QVariant::fromValue(m_display_qtc_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if qtc.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_qtc_unit);
...
{
    if (new_unit.isNull() || new_unit.value<Quantum CoinUnit>() == m_display_qtc_unit) return;
    m_display_qtc_unit = new_unit.value<Quantum CoinUnit>();
    QSettings settings;
    settings.setValue("DisplayQuantum CoinUnit", QVariant::fromValue(m_display_qtc_unit));
    Q_EMIT displayUnitChanged(m_display_qtc_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/qtc/qtc/pull/8273
    ...
    // (https://github.com/qtc-core/gui/issues/567).
