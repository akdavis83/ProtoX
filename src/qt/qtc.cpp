...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<QTCUnit>("QTCUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. qtc_de.qm (shortcut "de" needs to be defined in qtc.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. qtc_de_DE.qm (shortcut "de_DE" needs to be defined in qtc.qrc)
...
static const char* qt_argv = "qtc-qt";

Quantum CoinApplication::Quantum CoinApplication()
...
void Quantum CoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the Quantum CoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QTCGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void Quantum CoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool Quantum CoinApplication::createOptionsModel(bool resetSettings)
{
...
void Quantum CoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new QTCGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &Quantum CoinGUI::quitRequested, this, &Quantum CoinApplication::requestShutdown);
...
void Quantum CoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void Quantum CoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool Quantum CoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void Quantum CoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &Quantum CoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &Quantum CoinApplication::handleRunawayException);
    connect(this, &Quantum CoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &Quantum CoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void Quantum CoinApplication::parameterSetup()
{
...
void Quantum CoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void Quantum CoinApplication::requestInitialize()
{
...
void Quantum CoinApplication::requestShutdown()
{
...
void Quantum CoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // qtc: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &Quantum CoinGUI::handlePaymentRequest);
            connect(window, &Quantum CoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void Quantum CoinApplication::handleRunawayException(const QString &message)
{
...
void Quantum CoinApplication::handleNonFatalException(const QString& message)
{
...
WId Quantum CoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool Quantum CoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", Quantum CoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(QTC_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see qtc-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see qtc-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse qtc.conf, determine network, switch to network specific
    ...
    // qtc: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
