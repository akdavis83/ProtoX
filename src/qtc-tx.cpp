...
static void SetupQuantum CoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " qtc-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The qtc-tx tool is used for creating and modifying qtc transactions.\n\n"
                "qtc-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded qtc transaction, or with \"-create [commands]\" to create a hex-encoded qtc transaction.\n"
                "\n"
                "Usage: qtc-tx [options] <hex-tx> [commands]\n"
                "or:    qtc-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded qtc transaction
