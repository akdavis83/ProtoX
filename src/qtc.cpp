...
Options:
  -m, --multiprocess     Run multiprocess binaries qtc-node, qtc-gui.
  -M, --monolithic       Run monolithic binaries qtcd, qtc-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show this help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'qtc-qt [ARGS]' or 'qtc-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'qtcd [ARGS]' or 'qtc-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'qtc-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'qtc-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'qtc-tx [ARGS]'.
...
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_qtc [ARGS]'.
  chainstate [ARGS] Run qtc kernel chainstate util, equivalent to running 'qtc-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_qtc [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_qtc-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "qtc-gui" : "qtc-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "qtc-node" : "qtcd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("qtc-cli");
            // Since "qtc rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("qtc-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("qtc-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_qtc");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("qtc-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_qtc");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_qtc-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("qtc-util");
        } else {
...
//! Execute the specified qtcd, qtc-qt or other command line in `args`
...
//!             be a relative path to a qtcd, qtc-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/qtc ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/qtc/qtc/pull/31375#discussion_r1861814807)
