                    ...
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::ARR, "addresses", "Only when connected = true",
                            {
                                {RPCResult::Type::OBJ, "", "",
                                {
                                    {RPCResult::Type::STR, "address", "The qtc server IP and port we're connected to"},
    ...
    return RPCHelpMan{"getnetworkinfo",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::OBJ, "pqnoise", "PQ Noise transport statistics",
                        {
                            {RPCResult::Type::NUM, "handshakes_attempted", "Total PQ handshake attempts"},
                            {RPCResult::Type::NUM, "handshakes_successful", "Successful PQ handshakes"},
                            {RPCResult::Type::NUM, "handshakes_failed", "Failed PQ handshakes"},
                            {RPCResult::Type::NUM, "bytes_encrypted", "Total bytes encrypted with PQ transport"},
                            {RPCResult::Type::NUM, "bytes_decrypted", "Total bytes decrypted with PQ transport"},
                            {RPCResult::Type::NUM, "rekeys_performed", "Number of transport rekeys performed"},
                            {RPCResult::Type::NUM, "sessions_active", "Currently active PQ sessions"},
                            {RPCResult::Type::STR, "suite", "PQ cipher suite in use"},
                        }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Add PQ Noise metrics
    if (node.connman && (node.connman->GetLocalServices() & NODE_PQNOISE)) {
        UniValue pq_metrics(UniValue::VOBJ);
        auto metrics = PQMetrics::GetMetrics();
        
        pq_metrics.pushKV("handshakes_attempted", metrics["handshakes_attempted"]);
        pq_metrics.pushKV("handshakes_successful", metrics["handshakes_successful"]);
        pq_metrics.pushKV("handshakes_failed", metrics["handshakes_failed"]);
        pq_metrics.pushKV("bytes_encrypted", metrics["bytes_encrypted"]);
        pq_metrics.pushKV("bytes_decrypted", metrics["bytes_decrypted"]);
        pq_metrics.pushKV("rekeys_performed", metrics["rekeys_performed"]);
        pq_metrics.pushKV("sessions_active", metrics["sessions_active"]);
        pq_metrics.pushKV("suite", "NoisePQ_KYBER1024_DILITHIUM3_SHA3-512_CHACHA20-POLY1305");
        
        obj.pushKV("pqnoise", std::move(pq_metrics));
    }
