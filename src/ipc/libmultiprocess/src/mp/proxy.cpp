// Copyright (c) 2019 The QTC Core developers
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking qtc code so they have to run
    ...
    // functions run, so client objects are fully disconnected before qtc
    // code in the destructors are run. This way if the qtc code tries to
...
void Connection::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In qtc core, running these callbacks in the right order is
